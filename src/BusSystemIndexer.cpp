...
struct CBusSystemIndexer::SImplementation{
    ...
    // Stops sorted ascending by stop ID for O(1) index lookup
    std::vector<std::shared_ptr<SStop>> DSortedStopsByIndex;
    // Fast node-ID → stop lookup
    ...
    // Custom hash for (src, dest) node-ID pairs used as route adjacency keys
    ...
    // Maps a consecutive stop pair (by node ID) to all routes that traverse it
    ...
    SImplementation(std::shared_ptr<CBusSystem> bussystem){
        ...
        // Index each consecutive stop pair on every route for fast route-between-nodes queries
